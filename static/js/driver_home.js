...
function displayRecommendedRide(ride) {
    ...
    // NOVO: mostrar botão concluir se a viagem já foi aceite (ride_date != null)
    if (ride.ride_date !== null) {
...
function finishRide() {
    ...
    .then(data => {
        if (data.success) {
            alert("🏁 Concluded Ride!");
            loadRides();
        } else {
            alert("Error concluding ride: " + data.error);
        }
    })
    .catch(err => {
        console.error("Error:", err);
        alert("Error concluding ride");
    });
}
